...
__global__ void no_2e_tmpA_os_kernel(int n_grid1, int n_mo, int ne_b, int ne_a,
    ...
    while(i_grid1 < n_grid1) {
        ...
        for(p_mo = 0; p_mo < n_mo; p_mo++) {
            ...
            for(ie = ne_b; ie < ne_a; ie++) {
                ...
                tmpA[ix] += 0.5 * wr1_tmp * mol_tmp * int2_grad1_u12[iix            ];
                tmpA[iy] += 0.5 * wr1_tmp * mol_tmp * int2_grad1_u12[iix +   n_grid1];
                tmpA[iz] += 0.5 * wr1_tmp * mol_tmp * int2_grad1_u12[iix + 2*n_grid1];
...
extern "C" void no_2e_tmpA_os(int n_grid1, int n_mo, int ne_b, int ne_a,
                              double * wr1, double * mos_l_in_r, double * int2_grad1_u12,
                              double * tmpA) {
    ...
    printf("lunching no_2e_tmpA_os_kernel with %d blocks and %d threads/block\n", nBlocks, blockSize);

    no_2e_tmpA_os_kernel<<<nBlocks, blockSize>>>(n_grid1, n_mo, ne_b, ne_a,
