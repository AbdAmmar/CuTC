...
__global__ void no_1e_tmpC_os_kernel(int n_grid1, int n_mo, int ne_b, int ne_a,
        ...
        for(p_mo = 0; p_mo < n_mo; p_mo++) {
            ...
            for(s_mo = 0; s_mo < n_mo; s_mo++) {
                ...
                for(ie = ne_b; ie < ne_a; ie++) {
                    ...
                    tmpC[iC + 3*n_grid1] += 0.5 * int2_grad1_u12[jjx            ] * int2_grad1_u12[kkx            ];
                    tmpC[iC + 3*n_grid1] += 0.5 * int2_grad1_u12[jjx +   n_grid1] * int2_grad1_u12[kkx +   n_grid1];
                    tmpC[iC + 3*n_grid1] += 0.5 * int2_grad1_u12[jjx + 2*n_grid1] * int2_grad1_u12[kkx + 2*n_grid1];
...
extern "C" void no_1e_tmpC_os(int n_grid1, int n_mo, int ne_b, int ne_a,
                              double * int2_grad1_u12,
                              double * tmpC) {
    ...
    no_1e_tmpC_os_kernel<<<nBlocks, blockSize>>>(n_grid1, n_mo, ne_b, ne_a,
                                                 int2_grad1_u12,
                                                 tmpC);
